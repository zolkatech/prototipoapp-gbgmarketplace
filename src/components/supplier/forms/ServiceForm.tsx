...
export default function ServiceForm({ supplierId, formData, onFormDataChange, onSubmit, loading, editingProduct }: ServiceFormProps) {
  ...
  const addServiceLocation = () => {
    if (newLocation.trim() && !formData.service_locations.includes(newLocation.trim())) {
      onFormDataChange({
        ...formData,
        service_locations: [...formData.service_locations, newLocation.trim()]
      });
      setNewLocation('');
    }
  };
  ...
  const removeServiceLocation = (location: string) => {
    onFormDataChange({
      ...formData,
      service_locations: formData.service_locations.filter(loc => loc !== location)
    });
  };
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="Ex: Ferrageamento especializado"
            required
          />
        </div>
      ...
      <div className="space-y-6">
        <div className="space-y-2">
          <Label>Imagens do Serviço (máx. 3)</Label>
        ...
        <div className="space-y-2">
          <Label>Vídeos do Serviço (máx. 3)</Label>
          <div className="flex items-center gap-3">
            <Input 
              type="file" 
              accept="video/*" 
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder="Descreva seu serviço, experiência e metodologia..."
          rows={4}
          required
        />
      </div>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-medium">Locais de Atendimento</h3>
        
        <div className="space-y-2">
          <Label>Onde presto o serviço</Label>
          <div className="flex flex-wrap gap-2 mb-2">
            {formData.service_locations.map((location, index) => (
          ...
          <div className="flex gap-2">
            <Input
              ...
              placeholder="Ex: Na propriedade do cliente, Minha clínica, etc."
              onKeyPress={(e) => {
                if (e.key === 'Enter') {
                  e.preventDefault();
                  addServiceLocation();
            ...
            />
            <Button type="button" onClick={addServiceLocation} size="sm">
      ...
      <div className="flex justify-end space-x-2">
        <Button type="submit" disabled={loading}>
          {loading ? 'Salvando...' : editingProduct ? 'Atualizar Serviço' : 'Adicionar Serviço'}
        </Button>
      </div>
    </form>
  );
